...
linelasreduced2d::linelasreduced2d(int *cinte, int cmregparm, long int celemno){
  nprop    = 2;
  nregprop = 2;
  mdofn    = 2;
  nonlinflag = false;
  this->mregparm = cmregparm;
  shpstrct = new shapestruct(2,2,cinte,4,LINELASREDUCED2D,celemno);
  ...
  stiffkr1    = allocmat(shpstrct->numnodes*mdofn,shpstrct->numnodes*mdofn,stiffkr1);
  stiffkr2    = allocmat(shpstrct->numnodes*mdofn,shpstrct->numnodes*mdofn,stiffkr2);
  stiffki     = allocmat(shpstrct->numnodes*mdofn,shpstrct->numnodes*mdofn,stiffki);
  ...
  for(int inode = 0;inode<shpstrct->numnodes;inode++){   dofnatnode[inode] = mdofn;   }
...
//==================================================================
linelasreduced2d::~linelasreduced2d(){
  ...
  deallocmat(shpstrct->numnodes*mdofn,stiffkr1);
  deallocmat(shpstrct->numnodes*mdofn,stiffkr2);
  deallocmat(shpstrct->numnodes*mdofn,stiffki);
...
//================================================================
//===============================================================
void linelasreduced2d::elemstiff(int which_stiffness){
  ...
  zeromat(shpstrct->numnodes*mdofn,shpstrct->numnodes*mdofn,stiffkr1);
  zeromat(shpstrct->numnodes*mdofn,shpstrct->numnodes*mdofn,stiffkr2);
  zeromat(shpstrct->numnodes*mdofn,shpstrct->numnodes*mdofn,stiffki);
  ...
  for (int iprop = 0; iprop < 2; iprop++){
   ...
   if(iprop == 0){
	...
	mu     += shpstrct->shape[iinte][inode]*prop[inode][1];
      ...
      if(iprop == 0){
	DD[0][0] = lambda;        DD[0][1] = lambda;        DD[0][2] = 0;
	DD[1][0] = lambda;        DD[1][1] = lambda;        DD[1][2] = 0;
	DD[2][0] = 0;             DD[2][1] = 0;             DD[2][2] = 0;
      }
      else{
	DD[0][0] = 2*mu;          DD[0][1] = 0;         DD[0][2] = 0;
	DD[1][0] = 0;             DD[1][1] = 2*mu;      DD[1][2] = 0;
	...
	double BA[3][2];
	double BIA[3][2];  // imaginary part, Hussein PRSA 2009
	...
	// Bloch peridoic Hussein type B matrices
	BIA[0][0] =  kvecx * shpstrct->shape[iinte][inode]; 
 	BIA[1][0] =  0; 
	BIA[2][0] =  kvecy * shpstrct->shape[iinte][inode]; 

	BIA[0][1] =  0; 
 	BIA[1][1] =  kvecy * shpstrct->shape[iinte][inode]; 
	BIA[2][1] =  kvecx * shpstrct->shape[iinte][inode]; 
	...
	for(int jnode =0; jnode<shpstrct->numnodes; jnode++){
	  ...
	  // Bloch periodic FE Mats
	  BIB[0][0] =  kvecx * shpstrct->shape[iinte][jnode]; 
	  BIB[1][0] =  0; 				    
	  BIB[2][0] =  kvecy * shpstrct->shape[iinte][jnode]; 
	  	                                              
	  BIB[0][1] =  0; 				    
	  BIB[1][1] =  kvecy * shpstrct->shape[iinte][jnode]; 
	  BIB[2][1] =  kvecx * shpstrct->shape[iinte][jnode]; 
	  ...
	  for(int idofn = 0;idofn<dofnatnode[inode];idofn++){
		...
		for(int jj=2;jj>=0;--jj){
		  ...
		  if ( which_stiffness == BUILD_REAL_STIFFNESS ) {
		    estiff[ieqn][jeqn] +=   BA[ii][idofn]*DD[ii][jj]*BB[jj][jdofn]*jacodet*weight;
		  }
		  
		  if ( which_stiffness == BUILD_COMPLEX_STIFFNESS_K2W ) {
          	    estiff[ieqn][jeqn] +=   BA[ii][idofn]*DD[ii][jj]*BB[jj][jdofn]*jacodet*weight;
		    estiff[ieqn][jeqn] +=  BIA[ii][idofn]*DD[ii][jj]*BIB[jj][jdofn]*jacodet*weight;
		    estiffimag[ieqn][jeqn] += BA[ii][idofn]*DD[ii][jj]*BIB[jj][jdofn]*jacodet*weight;
		    estiffimag[ieqn][jeqn] -= BIA[ii][idofn]*DD[ii][jj]*BB[jj][jdofn]*jacodet*weight;
		  }

		  if ( which_stiffness == BUILD_COMPLEX_STIFFNESS_W2K ){
		    // we can reuse the BA's because the kvecs have been normalized to be unit vectors
		    stiffkr1[ieqn][jeqn] +=   BA[ii][idofn]*DD[ii][jj]*BB[jj][jdofn]*jacodet*weight;
		    stiffkr2[ieqn][jeqn] +=  BIA[ii][idofn]*DD[ii][jj]*BIB[jj][jdofn]*jacodet*weight;
		    stiffki[ieqn][jeqn]  += BA[ii][idofn]*DD[ii][jj]*BIB[jj][jdofn]*jacodet*weight;
		    stiffki[ieqn][jeqn]  -= BIA[ii][idofn]*DD[ii][jj]*BB[jj][jdofn]*jacodet*weight;
		  }
...
//====================================================================
void linelasreduced2d::elemcmass(int which_mass){
...
//====================================================================
void linelasreduced2d::calcgradient(){
...
//====================================================================
void linelasreduced2d::getinteg(){
  gauss2d(shpstrct);
}
//====================================================================
void linelasreduced2d::getshape(){
  shape2d(shpstrct);
}
//====================================================================
void linelasreduced2d::printeltype(){
  //cout<<endl<<"This is LINELASREDUCED2D"<<endl;
}
//====================================================================
//void linelasreduced2d::memalloc(){

  // inherited
//}
//====================================================================
//void linelasreduced2d::memdealloc(){
//  // dealloc memory
//}
//===================================================================
void linelasreduced2d::eleminitstrn(){
  // zero right hand side vector
  zerovec(shpstrct->numnodes*mdofn,erhs);
  ...
  // this can be done using B^{T}D\epsilon_{0}, but we never calculate B^{T}D
  // we calculate the stiffness matrix B^{T}D so, we do the entire thing again
 ...
 // zero element stiffness K_{A,B} = \int_{\Omega}(N_{A,x}k(x)N_{B,X})
  ...
  //pick property to implement reduced integration
  // if iprop == 0 then we do selective integration on lambda 
  int tempninte[2],temptinte;
  tempninte[0] =shpstrct->ninte[0];
  tempninte[1] =shpstrct->ninte[1];
  temptinte    =shpstrct->tinte;
...
// ====================================================================================================
void linelasreduced2d::homogenizehassanihinton(){
  ...
  for (int iprop = 0; iprop < 2; iprop++){
    ...
    // Selective reduced integration (SRI) to calculate effective properties
    // SRI is only used to solve the equilibrium equations correctly.
	...
	for (int jj = 0; jj < shpstrct->ndime; jj++){
	  ...
	  double firstcontrib;
	  if ( iprop == 0){
	    firstcontrib  = ( lambda*tempfourth1  )*jacodet*weight;
	  }
	  if ( iprop == 1){
	    firstcontrib  = mu*tempfourth2*jacodet*weight;
	  }
	  ...
	  for (int pp = 0; pp < shpstrct->ndime; pp++){
		...
		khi_kl_pq += shpstrct->glbder[iinte][inode][qq]*this->primal[inode][pp];
	      ...
	      double eijpq;
	      if ( iprop == 0) {
		eijpq = lambda*(delta(ii,jj)*delta(pp,qq));
	      }
	      if ( iprop == 1) {
		eijpq =  mu*( delta(ii,pp)*delta(jj,qq) + delta(jj,pp)*delta(ii,qq));
	      }
