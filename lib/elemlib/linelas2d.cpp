...
linelas2d::linelas2d(int *cinte,int cmregparm,long int celemno){
  nprop = 2;
  mdofn = 2;
  this->mregparm = cmregparm;
  nonlinflag = false;
  shpstrct = new shapestruct(2,2,cinte,4,LINELAS2D,celemno);
...
//================================================================
void linelas2d::elemstiff(int which_stiffness){
...
void linelas2d::calcgradient(){
  ...
  for(int iinte = 0; iinte <shpstrct->tinte; iinte++){
	...
	if ( initstrntype == 0 ) {
	  ...
	  // calculate the gradient - did we confuse dofn and dime here? doesn't matter for now.
	  double fourthtensor;
	  for ( int ii = 0 ; ii < shpstrct->ndime; ii++){
		...
		for( int ll = 0; ll < shpstrct->ndime; ll++){
		  int ifieldkl;
		  if ( kk == ll ) { ifieldkl = kk;  } else {  ifieldkl = 2; }

		  double gijkl = 0.0;

		  if (this->which_functional == PENTANORM){
		    if ( this->BASE_PMIJKL[ii][jj][kk][ll] > -5.0 ){
		      gijkl     = (this->BASE_EHIJKL[ii][jj][kk][ll] - this->BASE_PMIJKL[ii][jj][kk][ll]);
		    }
		  }
		  ...
		  if ( this->BASE_PMIJKL[ii][jj][kk][ll] > 0.0 ) {
		    if (this->which_functional == PENTANORM2) {
		      gijkl     = (this->BASE_EHIJKL[ii][jj][kk][ll] - this->BASE_PMIJKL[ii][jj][kk][ll]);
		      gijkl     = gijkl /(this->BASE_PMIJKL[ii][jj][kk][ll]*this->BASE_PMIJKL[ii][jj][kk][ll]);
		    }
		    else if (this->which_functional == PENTANORM3 ) {
		      gijkl     = (this->BASE_EHIJKL[ii][jj][kk][ll] - this->BASE_PMIJKL[ii][jj][kk][ll]);
		      gijkl     = gijkl /this->BASE_PMIJKL[ii][jj][kk][ll];

		    }
		  }

		  double vklij     = 0.0;
		  double gijpqchipqkl = 0.0;
		  double vpqijchipqkl = 0.0;
 
		  for ( int inode  = 0; inode < shpstrct->numnodes; inode++){
		    vklij   += dualwhole[inode][ii][ifieldkl]*shpstrct->glbder[iinte][inode][jj];
		  }

		  if ( iprop == 0 ) {
		    fourthtensor = ( delta(ii,jj)*delta(kk,ll) );
		  }
		  else{
		    fourthtensor = ( delta(ii,kk)*delta(jj,ll) + delta(jj,kk)*delta(ii,ll));
		  }
		  ...
		  for ( int pp = 0 ; pp < shpstrct->ndime; pp++){
		    for ( int qq = 0; qq < shpstrct->ndime; qq++){
		     ...
		     if ( this->BASE_PMIJKL[ii][jj][pp][qq] > 0.0 ) {
		       gijpq   = (this->BASE_EHIJKL[ii][jj][pp][qq] - this->BASE_PMIJKL[ii][jj][pp][qq]);
		       //gijpq   = (this->BASE_EHIJKL[ii][jj][pp][qq] - this->BASE_PMIJKL[ii][jj][pp][qq])/this->BASE_PMIJKL[ii][jj][pp][qq];
		     }
		      ...
		      int ifieldpq;
		      // can be written elegantly as if (pp==qq) ifieldpq = pp else ifieldpq == 2;
		      if ( pp == qq ) { ifieldpq = pp;  } else {  ifieldpq = 2; }
		      
		      for ( int inode  = 0; inode < shpstrct->numnodes; inode++){
			vpqij   += dualwhole[inode][ii][ifieldpq]*shpstrct->glbder[iinte][inode][jj];
			chipqkl += primalwhole[inode][kk][ifieldpq]*shpstrct->glbder[iinte][inode][ll];
		      }

		      gijpqchipqkl += gijpq*chipqkl;
		      vpqijchipqkl += vpqij*chipqkl;
		  ...
		  double firstterm   = (gijkl - gijpqchipqkl)/(this->unitcellvolume);
		  double secondterm  = vklij  - vpqijchipqkl;
		  
		  this->gradient[knode][iprop] += (1.0/this->nfields)*(firstterm + secondterm)*fourthtensor*shpstrct->shape[iinte][knode]*jacodet*weight;
...
//====================================================================
void linelas2d::getinteg(){
  gauss2d(shpstrct);
}
//====================================================================
void linelas2d::getshape(){
  shape2d(shpstrct);
}
//====================================================================
void linelas2d::printeltype(){
  //cout<<endl<<"This is LINELAS2D"<<endl;
}
//====================================================================
//void linelas2d::memalloc(){
//
  // inherited
//}
//====================================================================
//void linelas2d::memdealloc(){
  // dealloc memory
//}
//===================================================================
void linelas2d::homogenizehassanihinton(){
  ...
  int ll = 0;  
  if (  !( ( initstrntype <= 3*(shpstrct->ndime-1) ) && (initstrntype >=0)  ) ){
    cout<<endl<<"INVALID VALUE FOR initstrntype in linelas2d.cpp, initstrntype = "<<initstrntype<<endl;
    exit(1);
  }
  
  if ( initstrntype == 1 ){ kk = 0; ll = 0; }
  if ( initstrntype == 2 ){ kk = 1; ll = 1; }
  if ( initstrntype == 3 ){ kk = 0; ll = 1; }
